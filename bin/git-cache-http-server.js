...
Main.clone = function(remote,local,callback) {
	...
	console.log("Full command: git clone " + remote + " " + local);
	console.log("clone command printed");
	var proc = js_node_ChildProcess.exec("git clone --mirror \"" + remote + "\" \"" + local + "\"",callback);
...
Main.lfsFetch = function(remote,local,callback) {
	js_node_ChildProcess.exec("git -C \"" + local + "\" remote set-url origin \"" + remote + "\"",function(err,stdout,stderr) {
		var proc = js_node_ChildProcess.exec("git -C \"" + local + "\" lfs fetch --all \"" + remote + "\"",callback);
		proc.stdout.on("data",function(data) {
			console.log(data);
		});
	});
		...
		var v = new Promise(function(resolve,reject) {
			...
			Main.fetch(remote,local,function(ferr,stdout,stderr) {
				if(ferr != null) {
					...
					console.log("INFO: updating via git fetch: success");
					resolve(null);
				}
			});
		}).then(function(result) {
			return new Promise(function(resolve1,reject1) {
				if(result == null && Main.lfs) {
					Main.lfsFetch(remote,local,function(lerr,stdout2,stderr2) {
						if(lerr != null) {
							resolve1("ERR: git lfs fetch exited with \n\t\t\t\t\t\t\t\t\tnon-zero status: " + lerr.code);
						} else {
							console.log("INFO: lfs fetch: success");
							resolve1(null);
						}
					});
				} else {
...
Main.main = function() {
	Main.version = "0.0.2";
	var options = js_npm_Docopt.docopt(Main.usage,{ version : Main.version});
	Main.cacheDir = options["--cache-dir"].replace(Main.removeLineEndingsReg.r,"");
	Main.listenPort = Std.parseInt(options["--port"]);
	Main.lfs = options["--lfs"];
...
Main.removeLineEndingsReg = new EReg("(\r\n\t|\n|\r\t|\r\n|\r)","gm");
